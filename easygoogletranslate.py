...
class EasyGoogleTranslateOne:
    def __init__(self, source_language='auto', target_language='', timeout=5):
        self.source_language = source_language
        self.target_language = target_language
        self.timeout = timeout
        self.pattern = r'(?m)class="(?:t0|result-container)">(.*?)<'
    ...
    def translate(self, text, target_language='', source_language='', timeout=''):
        ...
        if type(target_language) is list:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(self.make_request, target, source_language, text, timeout) for target in
                           target_language]
